...
BUILD_OUTPUT = ROOT / "target" / "sdk-fixtures" / "grabba" / "ci-build-output.txt"


def verify_sdk_grabba_build(output: pathlib.Path, manifest: pathlib.Path) -> str:
...
def verify_build_receipt(receipt: str | bytes, digest: str) -> None:
    expected = "Grabba.aex SHA-256: " + digest
...
def test_official_sdk_grabba_builds_unchanged_and_records_hash():
    completed = None
    if os.environ.get("AEXCOMPAT_SDK_GRABBA_PREBUILT") != "1":
    ...
    digest = verify_sdk_grabba_build(OUTPUT, RESULT)
...
def test_sdk_grabba_verifier_rejects_contract_mutations(tmp_path, field, invalid):
    output = tmp_path / "Grabba.aex"
    ...
    with pytest.raises(AssertionError):
        verify_sdk_grabba_build(output, manifest)
...
def test_prebuilt_mode_verifies_without_rebuilding(tmp_path, monkeypatch):
    ...
    build_output.write_text(
        "Grabba.aex SHA-256: " + digest + "\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("AEXCOMPAT_SDK_GRABBA_PREBUILT", "1")
    monkeypatch.setattr("test_sdk_grabba_fixture_build.OUTPUT", output)
    monkeypatch.setattr("test_sdk_grabba_fixture_build.RESULT", manifest)
    monkeypatch.setattr("test_sdk_grabba_fixture_build.BUILD_OUTPUT", build_output)
...
def test_build_receipt_requires_the_actual_digest(encoding):
    digest = "1" * 64
    receipt = ("Grabba.aex SHA-256: " + digest).encode(encoding)
    verify_build_receipt(receipt, digest)
    with pytest.raises(AssertionError):
