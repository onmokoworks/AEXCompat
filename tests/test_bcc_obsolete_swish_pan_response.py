"""Legacy BCC Swish Pan follows manual progress; not exact AE parity."""
import json
import os
import subprocess
...
def test_installed_bcc_obsolete_swish_pan_tracks_manual_percent(
    ...
    assert parameters[2]['choices'] == ['Auto', 'Pct. Done']
    assert parameters[3]['name'] == 'Percent Done'
    assert parameters[3]['minimum'] <= 0 < 50 < 100 <= parameters[3]['maximum']
    assert parameters[4]['name'] == 'Layer to Reveal'
    assert parameters[4]['kind'] == 'layer'
    ...
    for percent in (0, 50, 100):
        ...
        assert requested[2] == 2 and requested[3] == percent
