...
def test_layer_parameter_uses_relative_pinned_bundle_transport(tmp_path):
    ...
    # The sidecar keeps a bundle-relative path so the bundle stays portable and
    # no absolute host path leaks; the harness resolves it against its own bundle
    # root at execution time.
    assert request["assignments"] == [
        {
            "slot": 4,
            "layer": manifest["input"]["path"].replace("\\", "/"),
        }
    ]
