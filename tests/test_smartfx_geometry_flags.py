...
def test_l2_source_validates_extra_pixels_and_geometry_rects() -> None:
    ...
    # The old inline validation lambda must not survive alongside the named
    # function, or the two could drift apart.
    assert "auto valid_rect = [&](std::size_t offset)" not in source
