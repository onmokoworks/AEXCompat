...
// The property under test is which steps reach the plug-in: dispatch_render
// enters it through `draw`, `dispatch_selector`, and `close_ui`, and every one
// of those has to be skipped once the plug-in has refused its own setup
// (issue #725).
...
// the setup_error accessor is the only way the refusal gets out.
...
// A refused FRAME_SETUP has to surface as LifecycleResult::error. Before #725
// the refusal stayed inside the opaque lifecycle and this came back 0, so the
// caller dispatched RENDER into a frame that was never set up.
void a_refused_setup_reaches_the_caller() {
  ...
  check(host.click_calls == 0 && host.interpolate_calls == 0 &&
        ...
        "no further selector runs after a refused setup");

  // The teardown still has to run, carrying the refusal.
  const int32_t finished = finish_lifecycle(&host, state, lifecycle_hooks(), false);
  check(finished == 512, "finish_lifecycle returns the refusal");
  check(host.end_calls == 1 && host.end_saw_error == 512, "teardown saw the refusal");
}
...
// The accessor is required, not optional: a hook table missing it cannot be
// used to tell a refusal from success, so it fails closed rather than
// silently reverting to the #725 behavior.
