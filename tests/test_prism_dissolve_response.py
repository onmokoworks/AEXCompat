"""Prism Dissolve manual transition response; not exact AE parity."""
import json
import os
import subprocess
...
from test_bcc_transition_slot8_response import assert_transition, reveal_pixels
from test_render_fixture_semantic_response import HEIGHT, ROOT, WIDTH


def test_real_prism_dissolve_manual_response(tmp_path):
    plugin = os.environ.get('AEXCOMPAT_TEST_PRISM_DISSOLVE')
    if not plugin:
        pytest.skip('set AEXCOMPAT_TEST_PRISM_DISSOLVE to the local AEX')
    assert os.name == 'nt'
    plugin = Path(plugin)
