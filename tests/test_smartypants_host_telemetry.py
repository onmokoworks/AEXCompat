...
def test_smart_pre_render_exposes_bounded_thread_safe_guid_mix_callback():
    ...
    assert "std::atomic<uint32_t> guid_mix_in_calls{}" in SOURCE
...
def test_comp_suite_v10_slot_four_records_successes_and_rejections():
    ...
    assert ".comp_bg_color_rejections.fetch_add" in callback
    assert ".comp_bg_color_successes.fetch_add" in callback
