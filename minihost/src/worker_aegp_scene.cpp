...
constexpr std::size_t kAegpEffectParameterCapacity = 12;
...
constexpr std::array<AegpEffectParameterRecord, 5> kAegpProbeParameters{{
...
constexpr std::array<AegpEffectParameterRecord, 7> kAegpLevelsParameters{{
...
constexpr std::array<AegpInstalledEffectRecord, 3> kAegpInstalledEffects{{
...
static_assert(kAegpInstalledEffects[0].key != kAegpInstalledEffectKeyNone);
...
constexpr std::size_t kAegpLegacyEffectStreamCapacity = 16;
...
struct AegpItemSuite {
  ...
  decltype(&aegp_get_active_item) get_active_item;
  void* before_get_item_type[2]{};
  decltype(&aegp_get_item_type) get_item_type;
...
std::array<AegpTime, 3> g_aegp_layer_in_points{{{0, 30}, {0, 30}, {0, 30}}};
std::array<AegpTime, 3> g_aegp_layer_durations{{{300, 30}, {300, 30}, {300, 30}}};
...
struct AegpCollectionSuite {
  void* new_collection{};
  decltype(&aegp_dispose_collection) dispose_collection;
  decltype(&aegp_get_collection_count) get_count;
  decltype(&aegp_get_collection_item) get_by_index;
  void* push_back{};
  void* erase{};
};
...
struct AegpLayerTransferMode { int32_t mode{}; int32_t flags{}; int32_t track_matte{}; };
...
void* next_layer_render_options_handle() {
  const uint64_t generation = g_layer_render_options_generation.fetch_add(1);
  if (generation == 0 || generation > (std::numeric_limits<uintptr_t>::max)() / 4) return nullptr;
  return reinterpret_cast<void*>(static_cast<uintptr_t>((generation << 2) | 2));
}
bool snapshot_layer_render_options(void* handle, AegpLayerRenderOptionsValue& value) {
  std::lock_guard<std::mutex> lock(g_layer_render_options_mutex);
  const auto found = g_layer_render_options.find(layer_render_options_key(handle));
  if (!handle || found == g_layer_render_options.end()) return false;
  value = found->second;
  return true;
}
int32_t insert_layer_render_options(const AegpLayerRenderOptionsValue& value, void** output) {
  if (!output) return 4;
  *output = nullptr;
  void* handle = next_layer_render_options_handle();
  if (!handle) return 4;
  std::lock_guard<std::mutex> lock(g_layer_render_options_mutex);
  if (g_layer_render_options.size() >= kMaxLayerRenderOptions) return 4;
  try {
    if (!g_layer_render_options.emplace(layer_render_options_key(handle), value).second) return 4;
  } catch (const std::bad_alloc&) { return 4; }
  ++g_layer_render_options_created;
  *output = handle;
  return 0;
}
template <typename F>
int32_t mutate_layer_render_options(void* handle, F&& mutation) {
  std::lock_guard<std::mutex> lock(g_layer_render_options_mutex);
  const auto found = g_layer_render_options.find(layer_render_options_key(handle));
  if (!handle || found == g_layer_render_options.end()) {
    ++g_invalid_layer_render_options_operations;
    return 4;
  }
  mutation(found->second);
  return 0;
}
int32_t __cdecl new_layer_render_options(int32_t plugin_id, void* layer, void** output) {
  if (output) *output = nullptr;
  if (plugin_id <= 0 || aegp_layer_index(layer) < 0 || !output) {
...
int32_t __cdecl duplicate_layer_render_options(int32_t plugin_id, void* handle, void** output) {
  if (output) *output = nullptr;
  AegpLayerRenderOptionsValue value{};
  if (plugin_id <= 0 || !output || !snapshot_layer_render_options(handle, value)) {
    ++g_invalid_layer_render_options_operations; return 4;
  }
  value.owner_plugin_id = plugin_id;
  return insert_layer_render_options(value, output);
}
int32_t __cdecl dispose_layer_render_options(void* handle) {
  std::lock_guard<std::mutex> lock(g_layer_render_options_mutex);
  const auto found = g_layer_render_options.find(layer_render_options_key(handle));
  if (!handle || found == g_layer_render_options.end()) {
    ++g_invalid_layer_render_options_operations; return 4;
  ...
  ++g_layer_render_options_disposed;
  return 0;
}
int32_t __cdecl set_layer_render_time(void* handle, AegpTime value) {
  if (value.scale == 0) { ++g_invalid_layer_render_options_operations; return 4; }
  return mutate_layer_render_options(handle, [&](auto& options) { options.time = value; });
}
int32_t __cdecl get_layer_render_time(void* handle, AegpTime* value) {
  AegpLayerRenderOptionsValue options{};
  if (!value || !snapshot_layer_render_options(handle, options)) {
    ++g_invalid_layer_render_options_operations; return 4;
  }
  *value = options.time; return 0;
}
int32_t __cdecl set_layer_render_time_step(void* handle, AegpTime value) {
  if (value.scale == 0 || value.value <= 0) {
    ++g_invalid_layer_render_options_operations; return 4;
  }
  return mutate_layer_render_options(handle, [&](auto& options) { options.time_step = value; });
}
int32_t __cdecl get_layer_render_time_step(void* handle, AegpTime* value) {
  AegpLayerRenderOptionsValue options{};
  if (!value || !snapshot_layer_render_options(handle, options)) {
    ++g_invalid_layer_render_options_operations; return 4;
  }
  *value = options.time_step; return 0;
}
int32_t __cdecl set_layer_render_world_type(void* handle, int32_t value) {
  if (value < 1 || value > 3) { ++g_invalid_layer_render_options_operations; return 4; }
  return mutate_layer_render_options(handle, [&](auto& options) { options.world_type = value; });
}
int32_t __cdecl get_layer_render_world_type(void* handle, int32_t* value) {
  AegpLayerRenderOptionsValue options{};
  if (!value || !snapshot_layer_render_options(handle, options)) {
    ++g_invalid_layer_render_options_operations; return 4;
  }
  *value = options.world_type; return 0;
}
int32_t __cdecl set_layer_render_downsample(void* handle, int16_t x, int16_t y) {
  if (x <= 0 || y <= 0) { ++g_invalid_layer_render_options_operations; return 4; }
  return mutate_layer_render_options(handle, [&](auto& options) {
    options.downsample_x = x; options.downsample_y = y;
  });
}
int32_t __cdecl get_layer_render_downsample(void* handle, int16_t* x, int16_t* y) {
  AegpLayerRenderOptionsValue options{};
  if (!x || !y || !snapshot_layer_render_options(handle, options)) {
    ++g_invalid_layer_render_options_operations; return 4;
  }
  *x = options.downsample_x; *y = options.downsample_y; return 0;
}
int32_t __cdecl set_layer_render_matte(void* handle, int32_t value) {
  if (value < 0 || value > 2) { ++g_invalid_layer_render_options_operations; return 4; }
  return mutate_layer_render_options(handle, [&](auto& options) { options.matte = value; });
}
int32_t __cdecl get_layer_render_matte(void* handle, int32_t* value) {
  AegpLayerRenderOptionsValue options{};
  if (!value || !snapshot_layer_render_options(handle, options)) {
    ++g_invalid_layer_render_options_operations; return 4;
  }
