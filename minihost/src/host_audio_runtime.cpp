...
void Runtime::configure_admission(bool audio_only_mode, bool usage_advertised) {
  telemetry_.usage_advertised = usage_advertised; telemetry_.checkout_allowed = audio_only_mode || usage_advertised;
...
bool Runtime::lifetimes_balanced() const { return live_handle_count() == 0 && telemetry_.checkout_calls == telemetry_.checkin_calls; }
int Runtime::checkout(void* effect_ref, std::int32_t index, std::int32_t start_time, std::int32_t duration,
  ...
  // Check admission before source availability so unadvertised callbacks stay observably denied.
  if (!telemetry_.checkout_allowed) { ++telemetry_.rejected_unadvertised_checkouts; ++telemetry_.invalid_operations; return kCallbackFailure; }
  if (!effect_ref || !audio || *audio || !source_ || index != 0 || duration < 0 || time_scale == 0) { ++telemetry_.invalid_operations; return kCallbackFailure; }
  ...
  telemetry_.last_checkout_start_time = start_time; telemetry_.last_checkout_duration = duration; telemetry_.last_checkout_time_scale = time_scale; telemetry_.last_window_start_sample = window_start; telemetry_.last_window_sample_count = static_cast<std::int32_t>(window_count); telemetry_.last_window_silence_samples = static_cast<std::int32_t>(silence_frames); telemetry_.last_output_rate = rate; telemetry_.last_output_bytes_per_sample = bytes_per_sample; telemetry_.last_output_channels = channels; telemetry_.last_output_format = format; telemetry_.last_returned_sample_frames = static_cast<std::int32_t>(returned_frames);
