...
UtilitySuiteSelftestResult verify_suite_entry_guards_and_utility13() {
  ...
  const bool passed = ok && utility_v7_acquired && unsupported_slots_diagnosed &&
      suite_acquire_count() == acquires_before + 2 &&
      suite_release_count() == releases_before + 2 && suite_leases_balanced();
  return {passed, utility_v7_acquired, unsupported_slots_diagnosed};
}
