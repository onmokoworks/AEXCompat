...
// The `--self-test-pf-private-effect-suite` route reports both counts as
// literals in worker_fixed_selftest_routing.cpp, and tests/ asserts them.
// Changing either constant has to change that string too.
static_assert(kSlotCount == 32 && kHostSlotCount == 10,
              "update the self-test route's metadata literals as well");

// Bounds for the one implemented slot. Neither is AE's: the source is a bare
...
// above the observed use (parameter names, `dest_bytes` = 0x100).
...
struct Suite {
  ...
  void* unsupported_tail[kSlotCount - kUtf16ToMultibyteSlot - 1];
};
