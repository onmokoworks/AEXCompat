...
impl ClusterManifestTransport {
    pub fn write(repository: &Path, manifest: &ValidatedClusterManifest) -> io::Result<Self> {
        ...
        // file name, so the staging source must itself be named
        // cluster-manifest-v1.json; the nonce directory keeps concurrent
        // cluster sessions apart.
        let dir = root.join(format!("cluster-manifest-{nonce}"));
        fs::create_dir(&dir)?;
        let path = dir.join(CLUSTER_MANIFEST_SEALED_BASENAME);
