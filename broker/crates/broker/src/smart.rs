use crate::host_core::approved_artifact::{load, load_v2_load_tree, ApprovedArtifact};
...
fn expected(case_id: &str) -> Option<&'static str> {
    match case_id {
        ...
        "deep16_default" => {
            Some("FDC0BC732683E9353F9A855D6EA2589B17D43D29D7B538093B474BEC6D5AD026")
        }
        "float32_default" => {
            Some("D707B9B7BD7C923182A0BEFCA60985896E473AFF3D0191FC310FC07CAD3FE90B")
        }
        "gpu_fallback_float32" => {
            Some("D707B9B7BD7C923182A0BEFCA60985896E473AFF3D0191FC310FC07CAD3FE90B")
        }
        "error_missing_input" => {
            Some("346790DBFE3BE4137B9B0B36606E504BCE22FF351867A183DE2FE8481BE1006E")
        }
        "crash_null_output_world" => Some(""),
        "temporal_context" => {
            Some("19CEA826F356E0D94BC29FF10CB9E7F5A770FE5B288CB3D190A58372353102D9")
        }
        "partial_output_request" => {
            Some("19CEA826F356E0D94BC29FF10CB9E7F5A770FE5B288CB3D190A58372353102D9")
        }
        _ => None,
    }
...
pub fn run(
    ...
    let root = repository.join("target/smart-results");
    ...
    if !parent.canonicalize()?.starts_with(root.canonicalize()?) {
        return Err(invalid("output outside SmartFX root"));
    }
    ...
    let gpu_valid = case_id != "gpu_fallback_float32"
        || reports.iter().all(|(_, r)| {
            r.get("gpu_device_setup_error") == Some(&json!(0))
                && r.get("gpu_device_setdown_error") == Some(&json!(0))
                && r.get("gpu_render_possible") == Some(&Value::Bool(false))
                && r.get("gpu_render_dispatched") == Some(&Value::Bool(false))
        });
    let expected_error = case_id == "error_missing_input";
    let expected_crash = case_id == "crash_null_output_world";
    let temporal_valid = case_id != "temporal_context"
        || reports.iter().all(|(_, r)| {
            r.get("checkout_time") == Some(&json!(42))
                && r.get("checkout_time_step") == Some(&json!(2))
                && r.get("checkout_time_scale") == Some(&json!(24))
        });
    let roi_valid = case_id != "partial_output_request"
        || reports.iter().all(|(_, r)| {
    ...
    let error_valid = !expected_error
        || reports.iter().all(|(c, r)| {
            c.as_str() == "nonzero_exit"
                && r.get("pre_render_error") == Some(&json!(0))
                && r.get("smart_render_error")
                    .and_then(Value::as_i64)
                    .is_some_and(|e| e != 0)
                && r.get("guard_bytes_intact") == Some(&Value::Bool(true))
        });
    let crash_valid = !expected_crash || reports.iter().all(|(c, _)| c.as_str() == "crashed");
    let success_valid = expected_error
        || expected_crash
        || reports.iter().all(|(c, r)| {
            c.as_str() == "ok"
                && r.get("pre_render_error") == Some(&json!(0))
                && r.get("smart_render_error") == Some(&json!(0))
                && r.get("result_rects_valid") == Some(&Value::Bool(true))
                && r.get("guard_bytes_intact") == Some(&Value::Bool(true))
        });
    ...
    let mut file = OpenOptions::new()
        ...
        .create_new(true)
        .open(output)?;
    serde_json::to_writer_pretty(&mut file, &summary).map_err(|e| invalid(e.to_string()))?;
    file.write_all(b"\n")?;
    Ok(passed)
}

fn resolve_receipt_path(repository: &Path, path: &Path) -> PathBuf {
    if path.is_absolute() {
        path.to_path_buf()
    } else {
