...
fn cli_contract() -> serde_json::Value {
    serde_json::json!({
        ...
        "commands": [
            ...
            {
                "name": "--inspect-experimental-report",
                "argv": ["--inspect-experimental-report", "<aex>"],
                "result": "parameter-inspection-and-diagnostics-json",
                "note": "Preserves the legacy raw-array command while exposing path-free execution provenance."
            },
            {
...
mod artifact_cli_contract_tests {
    ...
    fn raw_and_exr_commands_publish_exact_argument_and_result_contracts() {
        ...
        assert_eq!(
            command("--inspect-experimental-report")["result"],
            "parameter-inspection-and-diagnostics-json"
        );
    }
}
...
fn main() -> eframe::Result {
        ...
        )
    {
        ...
        let parameters =
            typed_render_request_parameters(&parameters, &assignment_document, Some(request_path))
                .unwrap_or_else(|error| {
                    emit_host_request_validation_failure(&error, &parameter_metadata);
                    std::process::exit(1);
                });
    ...
    if args.len() == 3 && args[1] == "--inspect-experimental-report" {
        let plugin = Path::new(&args[2]);
        let hash = required_plugin_hash(plugin);
        match aexcompat_broker::image_render::inspect_experimental_with_diagnostics(
            &repository,
    ...
    if args.len() == 5 && args[1] == "--diagnose-experimental-forced-user-changed-request" {
        let plugin = Path::new(&args[2]);
        let slot = args[3].to_string_lossy().parse::<u32>().unwrap_or(0);
        let request_path = Path::new(&args[4]);
        let request_bytes = fs::read(request_path).unwrap_or_else(|error| {
