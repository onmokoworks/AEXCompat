    ...
    match dispatch {
        Win64ImportDispatch::LegacyImplemented(implementation) => match implementation {
            ...
            LegacyWin64Import::GetModuleHandleExA => {
                uc("write GetModuleHandleExA return", unicorn.mem_write(stub, &[0xc3]))?;
                uc(
            ...
            LegacyWin64Import::GetModuleFileNameW => {
                uc("write GetModuleFileNameW return", unicorn.mem_write(stub, &[0xc3]))?;
                uc(
            ...
            LegacyWin64Import::LoadLibraryW => {
                uc("write LoadLibraryW return", unicorn.mem_write(stub, &[0xc3]))?;
                uc(
...
fn emulate_get_module_file_name_w(unicorn: &mut Unicorn<'_, GuestState>) {
    ...
    let path = if module == 0 || Some(module) == image_base {
...
fn emulate_load_library_w(unicorn: &mut Unicorn<'_, GuestState>) {
    ...
    let basename = name
        ...
        .unwrap_or_default()
        .to_ascii_lowercase();
    let module = if basename == "kernel32.dll" {
        Some(WINDOWS_KERNEL32_MODULE_TOKEN)
    } else if basename == "guest-plugin.aex" {
    ...
    if count >= MAX_WINDOWS_MODULE_REFERENCES {
        fail(unicorn, ERROR_TOO_MANY_OPEN_FILES);
        return;
    }
    unicorn
        .get_data_mut()
