...
static void gen_aex_avx_sync(DisasContext *s, uint8_t action)
{
#ifdef TARGET_X86_64
    TCGContext *tcg_ctx = s->uc->tcg_ctx;
    int first = 0;
    int last = CPU_NB_REGS;
    int first_qword = 2;
    ...
    for (int reg = first; reg < last; reg++) {
        for (int qword = first_qword; qword < 4; qword++) {
            tcg_gen_st_i64(
                tcg_ctx, s->tmp1_i64, tcg_ctx->cpu_env,
                offsetof(CPUX86State, xmm_regs[reg].ZMM_Q(qword)));
        }
    }
    if (action >= 1 && action <= 16) {
        tcg_gen_ld_i32(tcg_ctx, s->tmp2_i32, tcg_ctx->cpu_env,
                       offsetof(CPUX86State, aex_avx_defined_mask));
        tcg_gen_ori_i32(tcg_ctx, s->tmp2_i32, s->tmp2_i32,
                        1u << (action - 1));
        tcg_gen_st_i32(tcg_ctx, s->tmp2_i32, tcg_ctx->cpu_env,
                       offsetof(CPUX86State, aex_avx_defined_mask));
    } else {
        tcg_gen_movi_i32(tcg_ctx, s->tmp2_i32, 0xffff);
        tcg_gen_st_i32(tcg_ctx, s->tmp2_i32, tcg_ctx->cpu_env,
                       offsetof(CPUX86State, aex_avx_defined_mask));
    }
#else
...
static target_ulong disas_insn(DisasContext *s, CPUState *cpu)
{
    ...
    gen_aex_avx_sync(s, uc_x86_avx_sync_for_pc(env->uc, pc_start));
    ...
    if (uc_code_hook_exists_bounded(env->uc, pc_start)) {
        ...
        insn_hook = true;
        gen_uc_tracecode_checked(tcg_ctx, 0xf1f1f1f1, UC_HOOK_CODE_IDX,
                                 env->uc, pc_start);
