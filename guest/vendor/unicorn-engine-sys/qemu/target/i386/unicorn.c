...
UNICORN_EXPORT
uc_err uc_x86_reset_avx_defined_mask(uc_engine *uc)
{
    if (!uc || !uc->cpu) {
        return UC_ERR_HANDLE;
    }
    ((CPUX86State *)uc->cpu->env_ptr)->aex_avx_defined_mask = 0;
    return UC_ERR_OK;
}

UNICORN_EXPORT
uint32_t uc_x86_get_avx_defined_mask(uc_engine *uc)
{
    if (!uc || !uc->cpu) {
        return 0;
    }
    return ((CPUX86State *)uc->cpu->env_ptr)->aex_avx_defined_mask;
}
