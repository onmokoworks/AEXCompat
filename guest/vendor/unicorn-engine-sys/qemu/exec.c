 ...
 */
static int compare_ram_offsets(const void *left, const void *right)
{
    ram_addr_t a = *(const ram_addr_t *)left;
    ram_addr_t b = *(const ram_addr_t *)right;
    return (a > b) - (a < b);
}
...
static ram_addr_t find_ram_offset(struct uc_struct *uc, ram_addr_t size)
{
    ...
    offsets = count <= SIZE_MAX / sizeof(*offsets)
        ? g_try_malloc(count * sizeof(*offsets)) : NULL;
    if (offsets) {
        RAMBLOCK_FOREACH(block) {
            offsets[index++] = block->offset;
        }
        qsort(offsets, count, sizeof(*offsets), compare_ram_offsets);
    }
    ...
    RAMBLOCK_FOREACH(block) {
        ...
        if (offsets) {
            ...
            /* Preserve the original no-allocation path on index OOM. */
            RAMBlock *next_block;
