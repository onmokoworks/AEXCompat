...
- キャッシュは worker exe / この DLL / **依存環境** のいずれかが変わると全体が無効化され、
  再 discovery される。依存環境とは「探索フォルダ直下 + スキャン対象フォルダ配下の `*.aex`
  以外の全ファイル」の (パス, mtime, サイズ) のダイジェストで、AE のアップデートが
  `dvacore.dll` を書き換えた場合も、後から helper DLL を置いた場合も検出できる
  (AEX 自身もホストも変わらないため、これが無いと古い結果が残り続ける)。
