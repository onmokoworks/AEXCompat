...
- スキャン対象フォルダが一時的に読めなかった / 既定フォルダ (AE・MediaCore) が一時的に
  見つからなかった場合 → そのフォルダのキャッシュエントリを消さない (「スキャンされなかった」を
  「無くなった」と解釈しない)

**まだカバーできていないケース**: AEX ファイル自体が差し替わったとき (AE 側のアップデート等) は
`(mtime,len)` 変化で未登録になり、その 1 起動は上記の消失が起きうる。
