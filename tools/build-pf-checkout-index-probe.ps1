...
$source = Join-Path $repository "instruments\pf-checkout-index-probe"
$build = Join-Path $repository "target\pf-checkout-index-probe-build"
...
if ($LASTEXITCODE -ne 0) { throw "PF checkout index probe configure failed" }
& $CMake --build $build --config $Configuration --target pf_checkout_index_probe
if ($LASTEXITCODE -ne 0) { throw "PF checkout index probe build failed" }

$artifact = Join-Path $build "$Configuration\pf_checkout_index_probe.aex"
if (-not (Test-Path -LiteralPath $artifact)) {
    throw "PF checkout index probe artifact was not produced: $artifact"
}
