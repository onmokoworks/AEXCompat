...
$source = Join-Path $repository "instruments\pf-smart-map-layer-probe"
$build = Join-Path $repository "target\pf-smart-map-layer-probe-build"
...
if ($LASTEXITCODE -ne 0) { throw "Smart map layer probe configure failed" }
& $CMake --build $build --config $Configuration --target pf_smart_map_layer_probe --clean-first
if ($LASTEXITCODE -ne 0) { throw "Smart map layer probe build failed" }
$artifact = Join-Path $build "$Configuration\pf_smart_map_layer_probe.aex"
