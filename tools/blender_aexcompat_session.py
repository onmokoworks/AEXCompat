...
def build_response(request: dict[str, Any]) -> dict[str, Any]:
    ...
    source_relative_path = plugin.get("source_relative_path")
    if source_relative_path is not None:
        if not isinstance(source_relative_path, str) or not source_relative_path or Path(source_relative_path).is_absolute():
