...
$source = Join-Path $repository "instruments\pf-parameter-echo-probe"
$build = Join-Path $repository "target\pf-parameter-echo-probe-build"
...
if ($LASTEXITCODE -ne 0) { throw "PF parameter echo probe configure failed" }
& $CMake --build $build --config $Configuration --target pf_parameter_echo_probe
if ($LASTEXITCODE -ne 0) { throw "PF parameter echo probe build failed" }

$artifact = Join-Path $build "$Configuration\pf_parameter_echo_probe.aex"
if (-not (Test-Path -LiteralPath $artifact)) {
    throw "PF parameter echo probe artifact was not produced: $artifact"
}
