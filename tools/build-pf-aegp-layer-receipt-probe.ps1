...
$source = Join-Path $repository "instruments\pf-aegp-layer-receipt-probe"
$build = Join-Path $repository "target\pf-aegp-layer-receipt-probe-build"
...
if ($LASTEXITCODE -ne 0) { throw "PF AEGP layer receipt probe configure failed" }
& $CMake --build $build --config $Configuration --target pf_aegp_layer_receipt_probe
if ($LASTEXITCODE -ne 0) { throw "PF AEGP layer receipt probe build failed" }
$artifact = Join-Path $build "$Configuration\pf_aegp_layer_receipt_probe.aex"
