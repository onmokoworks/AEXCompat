...
issue #1210). They need only a Ghidra install and a project that holds the
...
| script | what it does |
| --- | --- |
| `DumpDecomp.java` | decompile functions selected by string reference (`str:`), import reference (`imp:`), address, and one-level `callees` / `callers` expansion; for analysed programs |
