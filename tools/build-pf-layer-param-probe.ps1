...
$source = Join-Path $repository "instruments\pf-layer-param-probe"
$build = Join-Path $repository "target\pf-layer-param-probe-build"
...
if ($LASTEXITCODE -ne 0) { throw "PF layer param probe configure failed" }
& $CMake --build $build --config $Configuration --target pf_layer_param_probe
if ($LASTEXITCODE -ne 0) { throw "PF layer param probe build failed" }

$artifact = Join-Path $build "$Configuration\pf_layer_param_probe.aex"
if (-not (Test-Path -LiteralPath $artifact)) {
    throw "PF layer param probe artifact was not produced: $artifact"
}
