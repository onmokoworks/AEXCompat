...
$source = Join-Path $repository "instruments\pf-aegp-layer-options-probe"
$build = Join-Path $repository "target\pf-aegp-layer-options-probe-build"
...
if ($LASTEXITCODE -ne 0) { throw "PF AEGP layer options probe configure failed" }
& $CMake --build $build --config $Configuration --target pf_aegp_layer_options_probe
if ($LASTEXITCODE -ne 0) { throw "PF AEGP layer options probe build failed" }
$artifact = Join-Path $build "$Configuration\pf_aegp_layer_options_probe.aex"
