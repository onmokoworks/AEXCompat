...
$props = Join-Path $PSScriptRoot 'sdk-fixtures\grabba-v143.props'
$target = Join-Path $repoRoot 'target\sdk-fixtures\grabba'
...
$rr = Join-Path $target 'Grabba_PiPL.rr'
$rrc = Join-Path $target 'Grabba_PiPL.rrc'
...
$commands = @(
    ...
    "`"$msbuild`" `"$project`" /nologo /m /t:Rebuild /p:Configuration=Release /p:Platform=x64 /p:PlatformToolset=v143 /p:ForceImportBeforeCppTargets=`"$props`" /p:AEXCompatGrabbaPiPLRc=`"$pipRc`" /p:OutDir=`"$targetMsBuild`" /p:IntDir=`"$intermediateMsBuild`" /p:AE_PLUGIN_BUILD_DIR=`"$target`" /bl:`"$target\build.binlog`""
)
$buildBatch = Join-Path $target 'build-grabba.cmd'
...
if ($buildExitCode -ne 0) {
    throw "Grabba v143 build failed with exit code $buildExitCode. See $buildLog"
}
...
Write-Output "Grabba.aex SHA-256: $hash"
