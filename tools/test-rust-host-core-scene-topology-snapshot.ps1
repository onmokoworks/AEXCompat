...
if ([string]::IsNullOrWhiteSpace($BuildDirectory)) {
    $BuildDirectory = Join-Path $repoRoot "target\issue632-native"
}
...
$nativeSource = Join-Path $repoRoot `
    "tests\native\rust_host_core_scene_topology_snapshot_dual_run_selftest.cpp"
$oracleSource = Join-Path $repoRoot `
    "minihost\src\worker_aegp_scene_model.cpp"
$executable = Join-Path $buildRoot `
    "rust_host_core_scene_topology_snapshot_dual_run_selftest.exe"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "MSVC scene topology snapshot dual-run build failed"
    }
    if (-not (Test-Path -LiteralPath $executable -PathType Leaf)) {
        throw "Native scene topology executable was not produced"
    }
    $nativeOutput = @(& $executable $rustDll 2>&1)
    if ($LASTEXITCODE -ne 0) {
        throw "Native scene topology snapshot dual-run failed: " +
            ($nativeOutput -join " ")
    }
    $report = ($nativeOutput -join "`n") | ConvertFrom-Json
    if ($report.rust_host_core_scene_topology_snapshot_dual_run -ne
            "passed" -or
        $report.cpp_registry -ne $true -or
        $report.canonical -ne $true -or
        $report.overflow_fail_closed -ne $true) {
        throw "Native scene topology snapshot dual-run returned an invalid report"
    }
