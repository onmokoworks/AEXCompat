...
$source = Join-Path $repository "instruments\pf-smart-geometry-probe"
$build = Join-Path $repository "target\pf-smart-geometry-probe-build"
...
if ($LASTEXITCODE -ne 0) { throw "Smart geometry probe configure failed" }
& $CMake --build $build --config $Configuration --target pf_smart_geometry_probe --clean-first
if ($LASTEXITCODE -ne 0) { throw "Smart geometry probe build failed" }
$artifact = Join-Path $build "$Configuration\pf_smart_geometry_probe.aex"
