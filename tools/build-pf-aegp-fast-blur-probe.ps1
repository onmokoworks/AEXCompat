...
$source = Join-Path $repository "instruments\pf-aegp-fast-blur-probe"
$build = Join-Path $repository "target\pf-aegp-fast-blur-probe-build"
...
if ($LASTEXITCODE -ne 0) { throw "PF AEGP Fast Blur probe configure failed" }
& $CMake --build $build --config $Configuration --target pf_aegp_fast_blur_probe
if ($LASTEXITCODE -ne 0) { throw "PF AEGP Fast Blur probe build failed" }
$artifact = Join-Path $build "$Configuration\pf_aegp_fast_blur_probe.aex"
