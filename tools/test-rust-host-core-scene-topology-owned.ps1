...
if ([string]::IsNullOrWhiteSpace($BuildDirectory)) {
    $BuildDirectory = Join-Path $repoRoot "target\issue634-native"
}
...
$nativeSource = Join-Path $repoRoot `
    "tests\native\rust_host_core_scene_topology_owned_dual_run_selftest.cpp"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "MSVC Rust-owned topology snapshot dual-run build failed"
    }
    if (-not (Test-Path -LiteralPath $executable -PathType Leaf)) {
        throw "Native Rust-owned topology executable was not produced"
    }
    $nativeOutput = @(& $executable $rustDll 2>&1)
    if ($LASTEXITCODE -ne 0) {
        throw "Native Rust-owned topology snapshot dual-run failed: " +
            ($nativeOutput -join " ")
    }
    $report = ($nativeOutput -join "`n") | ConvertFrom-Json
    if ($report.rust_host_core_scene_topology_owned_dual_run -ne
            "passed" -or
        $report.cpp_registry -ne $true -or
        $report.rust_owned -ne $true -or
        $report.buffer_detached -ne $true -or
        $report.thread_bound -ne $true -or
        $report.namespace_fail_closed -ne $true) {
        throw "Native Rust-owned topology dual-run returned an invalid report"
    }
