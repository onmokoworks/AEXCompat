...
$source = Join-Path $repository "instruments\pf-smart-param-time-probe"
$build = Join-Path $repository "target\pf-smart-param-time-probe-build"
...
if ($LASTEXITCODE -ne 0) { throw "Smart param time probe configure failed" }
& $CMake --build $build --config $Configuration --target pf_smart_param_time_probe --clean-first
if ($LASTEXITCODE -ne 0) { throw "Smart param time probe build failed" }
$artifact = Join-Path $build "$Configuration\pf_smart_param_time_probe.aex"
