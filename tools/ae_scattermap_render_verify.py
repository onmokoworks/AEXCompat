#!/usr/bin/env python3
"""Verify an AE ScatterMap default PNG against the fixed ARGB8 oracle."""
...
try:
    from tools.scattermap_reference_oracle import gradient, render_default
except ModuleNotFoundError:  # Direct execution places tools/ on sys.path.
    from scattermap_reference_oracle import gradient, render_default
...
def verify(path: Path) -> dict[str, object]:
    ...
    return {
        "schema_version": 1,
        "case_id": "scattermap_default_argb8_16x12",
...
def main() -> int:
    ...
    report = verify(args.png)
    args.out.parent.mkdir(parents=True, exist_ok=True)
    with args.out.open("x", encoding="utf-8", newline="\n") as handle:
        json.dump(report, handle, indent=2, sort_keys=True)
        handle.write("\n")
    return 0 if report["pixel_match"] and report["non_identity_output"] else 1
