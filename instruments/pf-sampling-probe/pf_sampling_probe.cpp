...
// macros today (the only std::max uses name their template argument, so the
// function-like macro cannot expand), and nothing here uses anything
// WIN32_LEAN_AND_MEAN excludes. They keep the next use from failing to compile
// with an error that points at <algorithm> rather than at this include.
