...
# GraphQL is required for PullRequestReviewThread.isResolved. The reviewThreads
# page and each thread's first 100 comments are fetched in one query; only the
# top-level reviewThreads connection is paginated by gh --paginate. A nested
# comments page overflow is carried as `truncated` and remains fail-closed, so
# the loop does not issue one follow-up API call per thread.
fetch_review_threads() {
  ...
  jq -s '[.[].data.repository.pullRequest.reviewThreads.nodes[] | {isResolved, truncated:(.comments.pageInfo.hasNextPage // false), comments:[.comments.nodes[] | {id:.databaseId,user:{login:.author.login},body,created_at:.createdAt,path,line,original_line:(.originalLine // null),in_reply_to_id:(.replyTo.databaseId // null)}]}]' <<<"$out"
}
...
while true; do
  ...
  if [ -n "$(codex_finding_supersedes_clean "$find_ts" "$newest_clean")" ]; then
  ...
  if [ -n "$text_clean_ts" ] && [ -z "$(codex_finding_supersedes_clean "$find_ts" "$text_clean_ts")" ]; then
  ...
  if [ -n "$react_clean_ts" ] && [ -z "$(codex_finding_supersedes_clean "$find_ts" "$react_clean_ts")" ]; then
